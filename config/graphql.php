...
$exporters = [
    'App\GraphQL\Location\LocationExporter',
    'App\GraphQL\Area\AreaExporter',
    'App\GraphQL\City\CityExporter',
    'App\GraphQL\Region\RegionExporter',
    'App\GraphQL\Country\CountryExporter',
    'App\GraphQL\Coordinate\CoordinateExporter',
];

if (!function_exists('getGraphQLTypes')) {
    function getGraphQLTypes($exporters) {
        $types = [];
        foreach ($exporters as $exporter) {
            $types = array_merge($types, $exporter::getTypes());
        }
        return $types;
    }
}

if (!function_exists('getGraphQLQueries')) {
    function getGraphQLQueries($exporters) {
        $queries = [];
        foreach ($exporters as $exporter) {
            $queries = array_merge($queries, $exporter::getQueries());
        }
        return $queries;
    }
}

if (!function_exists('getGraphQLMutations')) {
    function getGraphQLMutations($exporters) {
        $mutations = [];
        foreach ($exporters as $exporter) {
            $mutations = array_merge($mutations, $exporter::getMutations());
        }
        return $mutations;
    }
}
...
return [
    ...
    'schemas' => [
        'default' => [
            'query' => getGraphQLQueries($exporters),
            'mutation' => getGraphQLMutations($exporters),
        ],
    ],
     ...
     * ]
     */
    'types' => getGraphQLTypes($exporters),
